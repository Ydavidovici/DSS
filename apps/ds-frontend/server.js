...
const AUTH_SERVICE_URL =
    process.env.AUTH_SERVICE_URL || "http://127.0.0.1:4000";
    ...
    const requireAuth = async (req, res, next) => {
        ...
        try {
            ...
            if (token) {
                ...
                console.log(
                    "[Gatekeeper] No token found in incoming browser cookies."
                );
            }
            ...
            console.log(
                `[Gatekeeper] Auth Service returned Status: ${response.status}`
            );
            console.log(`[Gatekeeper] Auth Service Message: ${responseBody}`);

            if (response.ok) {
                console.log(
                    "[Gatekeeper] Access Granted. Serving dashboard..."
                );
                next();
            } else {
                console.log(
                    "[Gatekeeper] Access Denied. Redirecting to /login..."
                );
                res.redirect("/login");
            }
        } catch (error) {
            console.error(
                `[Gatekeeper] Fetch completely failed: ${error.message}`
            );
            res.redirect("/login");
        }
    };
