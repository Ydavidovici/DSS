            ...
            () =>
                async (req: any, res: any, next: any) => {
                    try {
                        ...
                        const {payload} = await (verifyToken as any)(token, "access");
...
describe("Verify & UserInfo", () => {
    ...
    it("GET /verify accepts token via Cookie (Gateway Mode)", async () => {
        const token = await signUserAccessToken({
            userId: "u1",
            email: "cookie-user@ex.com",
        });
    ...
    it("GET /verify returns X-User-Id header for Nginx", async () => {
        const token = await signUserAccessToken({
            userId: "u123",
            email: "admin@ex.com",
        });
        ...
        expect(res.headers['x-user-id']).toBe('u123');
        expect(res.headers['x-user-email']).toBe('admin@ex.com');
    });
});
